...
import { Observable, combineLatest, filter, map, tap, zip } from "rxjs";
...
import { RouteContext, RouteData } from "@common/routes/routes";
...
export class MenuService{
    ...
    public items = combineLatest({items: this._allItems, role: this.userService.currentUser$.pipe(map(u => u?.roles ?? UserRoles.GUEST))}).pipe(
        tap(r => console.log(r)),
...
function buildMenuItem(route: Route, textHost: TextHost): Observable<MenuItemReach | RootItem> | undefined {
    ...
    if(children) return children.pipe(map(children => {return {type: 'root', children: children} }));
